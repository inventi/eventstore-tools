...
@Import(CatchupSubscriptionsIntegrationTest.Config::class)
@ActiveProfiles("test")
@DirtiesContext // ensures that eventStore bean is not reused, because each bean has a different test container port
class CatchupSubscriptionsIntegrationTest : EventStoreIntegrationTest(), WithAsyncHandlerAssertions, WithEventstoreOperations {
    @Autowired
